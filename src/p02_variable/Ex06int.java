...
//    byte b4 = b1 + b3;
    ...
    // 1. boolean 을 제외한 4byte 이하 자료형은 int로 변환 후 연산
    short s1 = 1000;
    short s2 = 1000;
    // 2. 표현 범위가 큰 것이 작은 것으로 올 때는 명시적 형 변환 !
    short s3 = (short) (s1 + s2); // short = int
    System.out.println(s3);
    int i1 = s1 + s2;
    long l1 = 200L; // long = long

    // 3. 표현 범위가 작은 것이 큰 것으로 올 때는 묵시적 형 변환!
    long l2 = 200; // long = int, long l2 = (long) 200;

    int i2 = (int) l2; // 큰범위가 작은 범위로 올때는 명시적 선언!
    long result = i2 + l1;  //long과 연산할 때  long 으로 형 변환!
    Utils.typeOf(result);
